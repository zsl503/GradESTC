...
class FedPAQClient(FedAvgClient):
    ...
    def pack_client_model(self, raw_model:dict[str, torch.Tensor] , global_model):
        ...
        for key in raw_model.keys():
            ...
            if raw_model[key].dtype == torch.long or ('running_var' in key) or ('running_mean' in key) or key not in self.compress_layer:
                quantized_model[key], scale[key] = (global_model[key] - raw_model[key]), 0
            else:
                quantized_model[key], scale[key] = self.quantizer.quantize(global_model[key] - raw_model[key])
