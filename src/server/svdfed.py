...
class SVDFedServer(FedAvgServer):
    ...
    def train_one_round(self):
        ...
        for client_id, package in clients_package.items():
            self.clients_compress_combin[client_id] = package['compress_combin']
        ...
        if self.fixed_adj_freq == 0:
            ...
            for key in self.server_compress_combin.setting_dict.keys():
                ...
                if key in self.request_full_grad_params_name:
                    self.logger.log(f"Layer {key}: First round")
                    continue
                print(f"Layer {key} avg_error_norm: {avg_error_norm}, total_error: {self.total_error_dict[key]} ,delta_error: {delta_error}")
                # z = self.kp * avg_error_norm + self.ki * self.total_error_dict[key]
