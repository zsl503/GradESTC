...
class FedAvgServer:
    def __init__(
        ...
        self.args = args
        self.algo = f'{args.common.dataset}-{algo}-{args.common.desc}'
        ...
        if args.common.output_dir is not None:
            global OUT_DIR
            OUT_DIR = OUT_DIR / args.common.output_dir
        self.output_dir = OUT_DIR / self.algo / start_time
        if args.common.data_dir is None:
            raise ValueError("Please specify the data directory. In the config file, set `data_dir` to the path of the data directory.")
        # self.dataset_dir = "dataset" / args.common.data_dir
        self.dataset_dir = os.path.join("data", "dataset", args.common.data_dir)
        with open(
            FLBENCH_ROOT / self.dataset_dir / self.args.common.dataset / "args.json", "r"
        ) as f:
        ...
        try:
            partition_path = (
                FLBENCH_ROOT / self.dataset_dir / self.args.common.dataset / "partition.pkl"
            )
        ...
        self.verbose = False
        stdout = Console(log_path=False, log_time=False, width=240)
        self.logger = Logger(
            stdout=stdout,
    ...
    def init_trainer(self, fl_client_cls=FedAvgClient, **extras):
        """Initiate the trainier that responsible to client training.
        ...
        which are essential for all methods in this project.
    ...
    def get_client_dataset(self) -> BaseDataset:
        ...
        dataset: BaseDataset = DATASETS[self.args.common.dataset](
            root=FLBENCH_ROOT / self.dataset_dir / self.args.common.dataset,
    ...
    def train_one_round(self):
        ...
        for client_id in selected_clients:
            ...
            save_model_param(clients_package[client_id]['model_params_diff'],
                self.current_epoch,
                f'c{client_id}',
                is_grad=True,
    ...
    def run(self):
        """The entrypoint of experiment.

        Raises:
            RuntimeError: When bench trainer is not set properly.
        """
        begin = time.time()
        self.train()
        end = time.time()
