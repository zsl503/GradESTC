...
from client.gradestc import GradESTCClient
...
class GradESTCServer(FedAvgServer):
    @staticmethod
    ...
    def __init__(
        ...
        self.logger.log("Begin to initialize GradESTCServer.")
        self.init_trainer(GradESTCClient)
        if args.GradESTC.upload_setting_dict is None:
            upload_setting_dict = {}
        else:
            upload_setting_dict = args.GradESTC.upload_setting_dict.to_dict()
        if args.GradESTC.broadcast_setting_dict is None:
            broadcast_setting_dict = {}
        else:
            broadcast_setting_dict = args.GradESTC.broadcast_setting_dict.to_dict()
        u_dtype = args.GradESTC.u_type
    ...
    def train_one_round(self):
        ...
        self.test_model.load_state_dict(self.public_model_params, strict=False)
        clients_package = self.trainer.train()
        
        for client_id in selected_clients:
            self.clients_broadcast_compress_combin[client_id] = clients_package[client_id]['broadcast_compress_combin']
            self.clients_upload_compress_combin[client_id] = clients_package[client_id]['upload_compress_combin']
            ...
            # Save the model parameters for each round
    ...
    def aggregate(self, clients_package: OrderedDict[int, dict[str, Any]]):
        ...
        # 用于broadcast的
        weights = (torch.tensor(clients_weight) / sum(clients_weight)).cpu()
        for name, global_param in self.public_model_params.items():
            global_param.data = global_param.data.to(self.device)
            diffs = torch.stack(
                [
                    package["model_params_diff"][name].cpu()
